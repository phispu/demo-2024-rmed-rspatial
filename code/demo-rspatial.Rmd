...
- *Median household income*: We can see that the variables B06011_001 and B07011_001 are estimates for the median household income for the total population

```{r}
variables_2021_tract %>% 
  dplyr::filter(concept %>% stringr::str_detect('MEDIAN INCOME'))

variables_2021 %>% dplyr::filter(name == 'B07011_001')
```


Todo: add in educational attainment

```{r}
variables_2021_tract %>% 
  dplyr::filter(concept %>% stringr::str_detect('EDUCATION')) %>% view()

variables_2021_tract %>% filter(name %>% str_detect('B16010'))

# adding something, take 2

# variables_2021_tract %>% filter(name %>% str_detect('B16010'))
# B07011_001 and educational attainment (B16010_001, B16010_002, B16010_015, B16010_028, B16010_041)
```
...
```{r}
# TO DO: add image of search bar
variables_2021 %>% view()
```


### C.1.2. Import using get_acs(): A snapshot of 2021
...
#let's use median income in the past 12 months B06011_001 and educational attainment (B16010_001, B16010_002, B16010_015, B16010_028, B16010_041)
  # NOTE to CM: replaced B07011_001 b/c B06011_001 shows up earlier. Any reason why you chose B07011_001? I saw that both values seem to be the same.
...
ggplot(education_data, aes(x = education_level, y = proportion, fill = education_level)) +
...
## C.2. Range of Data: Median Household Income from 2009 to 2021
...
## C.3. CDC PLACES Data (health outcomes)
...
### ... Adding in Fill Colors


```{r}
smap_1 <- merged %>% 
  ggplot() +
  ggplot2::geom_sf(aes(fill = perc_hs))
smap_1 
